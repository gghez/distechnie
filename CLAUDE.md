...
├── css/
│   ├── custom.css                   # Site styles (WIP banner, EPUB links, etc.)
│   └── epub.css                     # EPUB typography (justify, hyphens, indents)
├── scripts/
...
<div class="epub-download">
  ...
  <a href="../epub/<story-slug>.kepub.epub" download class="kobo">⬇ Version Kobo</a>
</div>
...
- EPUB files: `<story-slug>.epub` and `<story-slug>.kepub.epub` (auto-generated in `book/epub/`, never committed)

## EPUB Generation

EPUBs are generated by `scripts/generate-epub.sh` using pandoc and kepubify:
...
- Converts each `.epub` to `.kepub.epub` (Kobo-optimized format) via kepubify
- Output goes to `book/epub/<story-slug>.epub` and `book/epub/<story-slug>.kepub.epub`
...
4. **Install kepubify** (for Kobo-optimized EPUB)
5. **Build book** — `mdbook build` outputs to `book/`
6. **Generate EPUBs** — runs `scripts/generate-epub.sh` (after build, into `book/epub/`)
7. **Deploy** — pushes `book/` to GitHub Pages

## Custom Styles

`css/custom.css` provides styles for the mdBook site:
- `.wip-banner` — yellow/amber banner for stories in progress (adapts to light/dark themes)
- `.epub-download` — styled download buttons (standard EPUB in blue, Kobo version in red)
...
`css/epub.css` provides typography for generated EPUBs (injected via `--css` in pandoc):
- Justified text with automatic hyphenation (`hyphens: auto`)
- Paragraph first-line indent (classic book style)
- Widows/orphans control, scene break styling
- No hardcoded colors or fonts — respects e-reader user settings
- Based on [epub-css-baseline](https://github.com/ai-autocoder/epub-css-baseline)

## Workflow Rules

### Loading Story Context

**Before writing or editing any story content, always read the story's `guidelines.md` first.** This file contains the creative guidelines (concept, characters, tone, structure, writing principles) that must be followed for consistency.

The `/story <slug>` skill automates this: it loads the guidelines, shows the current status (chapters, WIP, draft entries), and prepares the session. Use it at the start of any writing session. If no slug is provided, the skill lists available stories and asks which one to load.

Even without the skill, the rule applies: never write a chapter without having the guidelines in context.
